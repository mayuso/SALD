...
void SandboxApp::Run()
{
    ...
    while (!m_MainWindow->GetShouldClose())
    {
        ...
        camera.MouseControl(m_MainWindow->GetXChange(), m_MainWindow->GetYChange());

        if (m_MainWindow->GetKeys()[GLFW_KEY_L])
        {
            spotLights[0].Toggle();
            m_MainWindow->GetKeys()[GLFW_KEY_L] = false;
        }
...
void SandboxApp::RenderScene(Sald::Shader &currentShader)
{
...
void SandboxApp::DirectionalShadowMapPass(Sald::DirectionalLight *light)
{
...
void SandboxApp::OmniShadowMapPass(Sald::PointLight *light)
{
