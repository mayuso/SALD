...
Sald::Shader::Shader() : m_RendererID(0) {}

Sald::Shader::~Shader() {}

void Sald::Shader::CreateFromFiles(const char *vertexLocation,
...
void Sald::Shader::Bind() { glUseProgram(m_RendererID); }
void Sald::Shader::Unbind() { glUseProgram(0); }
...
void Sald::Shader::CompileShader(const char *vertexCode,
                                 const char *fragmentCode) {
  CompileShader(vertexCode, nullptr, fragmentCode);
}
...
void Sald::Shader::AddShader(GLuint theProgram, const char *shaderCode,
                             GLenum shaderType) {
...
void Sald::Shader::SetFloat3(const std::string &name, GLfloat x, GLfloat y,
                             GLfloat z) {
  GLint location = GetUniformLocation(name);
...
GLint Sald::Shader::GetUniformLocation(const std::string &name) {
  if (m_UniformLocationCache.find(name) != m_UniformLocationCache.end())
    return m_UniformLocationCache[name];

  GLint location = glGetUniformLocation(m_RendererID, name.c_str());
  if (location == -1)
    SALD_CORE_WARN("Warning: uniform '{0}' doesn't exist!", name);
